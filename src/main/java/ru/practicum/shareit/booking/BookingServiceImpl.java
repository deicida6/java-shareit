...
public class BookingServiceImpl implements BookingService {
    ...
    @Transactional
    @Override
    public BookingRequestDto approveOrRejectBooking(Long bookingId, Long userId, boolean approved) {
        Booking booking = bookingRepository.findById(bookingId).orElseThrow(() ->
                new NotFoundException("Не найдено бронирование с id = " + bookingId));
        if (!booking.getItem().getOwner().getId().equals(userId)) {
            throw new NotFoundException("Пользователя или нет или он не владелец вещи");
        }
    ...
    public BookingRequestDto getById(Long bookingId, Long userId) {
        ...
        if (!booking.getBooker().getId().equals(userId) && !booking.getItem().getOwner().getId().equals(userId)) {
            throw new NotFoundException("Пользователь (id = " + userId + ") не арендовывал вещь (id = " + bookingId + ")");
        }
